...
```{r, md_2, echo=FALSE, eval=TRUE, comment="", fig.pos="h!", fig.cap="25 Movies with the most ratings per year and their average rating post 1993\\label{fig:25_movies_avg_and_most_ratings_per_year_post_1993}"}
...
**Post-1993 movies**   
```{r, md_3, echo=FALSE, eval=TRUE, comment="", fig.pos="h!", fig.cap="Movies average ratings versus ratings per year post  1993\\label{fig:movies_average_ratings_versus_ratings_per_year_post_1993}"}
res3 <- edx_md %>%
  filter(movie_dt >= 1993) %>%
  group_by(movieId) %>%
  summarize(avg_rating = mean(rating), n = n(), title=title[1], years=2018 - min(movie_dt)) %>%
...
```{r, md_4, echo=FALSE, eval=TRUE, comment="", fig.pos="h!", fig.cap="Movies average ratings versus ratings per year pre  1993\\label{fig:movies_average_ratings_versus_ratings_per_year_pre_1993}"}
res3_b <- edx_md %>%
  filter(movie_dt < 1993) %>%
  group_by(movieId) %>%
  summarize(avg_rating = mean(rating), n = n(), title=title[1], years=2018 - min(movie_dt)) %>%
...
**_All Years_**
```{r, md_5, echo=FALSE, eval=TRUE, comment="", fig.pos="h!", fig.cap="Movies average ratings versus ratings per year for all years for edx\\label{fig:movies_average_ratings_versus_ratings_per_year_for_all_years_for_edx}"}
res3_all_edx <- edx_md %>%
  group_by(movieId) %>%
...
```{r, md_6, echo=FALSE, eval=TRUE, comment=""}
...
```{r, me_1, echo=1:2, eval=TRUE, collapse = TRUE, comment="", highlight=TRUE, background='#F7F7F7',  tidy.opts=list(blank = FALSE, width.cutoff = 60)}
...
```{r, me_2, echo=FALSE, eval=TRUE, comment="", fig.pos="h!", fig.cap="Movie effect or bias distribution\\label{fig:model_2}"}
movie_avgs %>%  
  ggplot(aes(b_i)) + 
...
```{r, me_3, echo=1:2, eval=TRUE, collapse = TRUE, comment="", highlight=TRUE, background='#F7F7F7',  tidy.opts=list(blank = FALSE, width.cutoff = 60)}
...
```{r, ue_1, echo=TRUE, eval=TRUE, collapse = TRUE, comment="", highlight=TRUE, background='#F7F7F7',  tidy.opts=list(blank = FALSE, width.cutoff = 60), fig.pos="h!", fig.cap="Average rating for users who have rated over 100 movies\\label{fig:average_ratings_for_users_who_have_rated_over_100_movies}"}
train_set %>% 
  group_by(userId) %>% 
...
```{r, ue_2, echo=TRUE, eval=TRUE, collapse = TRUE, comment="", highlight=TRUE, background='#F7F7F7',  tidy.opts=list(blank = FALSE, width.cutoff = 60), fig.pos="h!", fig.cap="Average rating for users who have rated any movies\\label{fig:average_ratings_for_users_who_have_rated_any_movies}"}
train_set %>% 
  group_by(userId) %>% 
  summarize(b_u = mean(rating)) %>% 
  ggplot(aes(b_u)) + 
...
\newpage
```{r, ue_3, echo=1:1, eval=TRUE, collapse = TRUE, comment="", highlight=TRUE, background='#F7F7F7',  tidy.opts=list(blank = FALSE, width.cutoff = 60)}
user_avgs <- train_set %>% 
  left_join(movie_avgs, by='movieId') %>%
  group_by(userId) %>%
...
```{r, ue_5, echo=1:2, eval=TRUE, collapse = TRUE, comment="", highlight=TRUE, background='#F7F7F7',  tidy.opts=list(blank = FALSE, width.cutoff = 60)}
...
```{r, ge_1, echo=TRUE, eval=TRUE, collapse = TRUE, comment="", highlight=TRUE, background='#F7F7F7',  tidy.opts=list(blank = FALSE, width.cutoff = 60), fig.pos="h!", fig.cap="Average rating for movies of category genres\\label{fig:average_ratings_for_movies_of_category_genres}"}
train_set %>% 
  group_by(genres) %>% 
...
```{r, ge_3, echo=TRUE, eval=TRUE, collapse = TRUE, comment="", highlight=TRUE, background='#F7F7F7',  tidy.opts=list(blank = FALSE, width.cutoff = 60)}
...
```{r, ge_4, echo=FALSE, eval=TRUE, comment="", fig.pos="h!", fig.cap="Genres effect or bias distribution\\label{fig:model_4}"}
genres_avgs %>%  
  ggplot(aes(b_g)) + 
  geom_histogram(bins = 10, color = "black") + 
  # scale_x_log10() +  # try with and without this line
...
```{r, ge_5, echo=1:2, eval=TRUE, collapse = TRUE, comment="", highlight=TRUE, background='#F7F7F7',  tidy.opts=list(blank = FALSE, width.cutoff = 60)}
...
```{r, rte_1, echo=TRUE, eval=TRUE, collapse = TRUE, comment="", highlight=TRUE, background='#F7F7F7',  tidy.opts=list(blank = FALSE, width.cutoff = 60)}
...
```{r, rte_2, echo=FALSE, eval=TRUE, comment="", fig.pos="h!", fig.cap="Rating time effect or bias distribution\\label{fig:model_5}"}
time_effect_avgs %>%  
  ggplot(aes(b_d)) + 
  geom_histogram(bins = 10, color = "black") + 
  # scale_x_log10() +  # try with and without this line
...
```{r, rte_3, echo=1:2, eval=TRUE, collapse = TRUE, comment="", highlight=TRUE, background='#F7F7F7',  tidy.opts=list(blank = FALSE, width.cutoff = 60)}
...
```{r, rde_1, echo=1:1, eval=TRUE, collapse = TRUE, comment="", highlight=TRUE, background='#F7F7F7',  tidy.opts=list(blank = FALSE, width.cutoff = 60)}
...
```{r, rde_2, echo=FALSE, eval=TRUE, comment="", fig.pos="h!", fig.cap="Release Date effect or bias distribution\\label{fig:model_6}"}
rel_effect_avgs %>%  
  ggplot(aes(b_r)) + 
  geom_histogram(bins = 10, color = "black") + 
  # scale_x_log10() +  # try with and without this line
...
```{r, rde_3, echo=1:3, eval=TRUE, comment=""}
...
```{r, reg_1, echo=TRUE, eval=TRUE, collapse = TRUE, comment="", highlight=TRUE, background='#F7F7F7',  tidy.opts=list(blank = FALSE, width.cutoff = 60)}
test_set %>% 
  left_join(movie_avgs, by='movieId') %>%
...
```{r, reg_2, echo=2:2, eval=TRUE, collapse = TRUE, comment="", highlight=TRUE, background='#F7F7F7', tidy.opts=list(blank = FALSE, width.cutoff = 60)}
load("rdas/edx.rda")
movie_titles <- edx %>% 
...
```{r, reg_3, echo=TRUE, eval=TRUE, collapse = TRUE, comment="", highlight=TRUE, background='#F7F7F7', tidy.opts=list(blank = FALSE, width.cutoff = 60)}
...
```{r, reg_4, echo=TRUE, eval=TRUE, collapse = TRUE, comment="", highlight=TRUE, background='#F7F7F7', tidy.opts=list(blank = FALSE, width.cutoff = 60)}
movie_avgs %>% left_join(movie_titles, by="movieId") %>%
  arrange(b_i) %>% 
  slice(1:10)  %>% 
  pull(title)%>% 
...
```{r, reg_5, echo=TRUE, eval=TRUE, collapse = TRUE, comment="", highlight=TRUE, background='#F7F7F7', tidy.opts=list(blank = FALSE, width.cutoff = 60)}
...
```{r, reg_6, echo=TRUE, eval=TRUE, message=FALSE, collapse = TRUE, comment="", highlight=TRUE, background='#F7F7F7', tidy.opts=list(blank = FALSE, width.cutoff = 60)}
train_set %>% count(movieId) %>% 
...
```{r, m7_1, echo=TRUE, eval=TRUE, comment=""}
...
```
```{r, m7_2, echo=FALSE, eval=TRUE}
rm(just_the_sum)
```

To see how the estimates shrink, let's make a plot of the regularized estimates versus the least squares estimates.

```{r, m7_3, echo=FALSE, eval=TRUE, collapse = TRUE, comment="", highlight=TRUE, background='#F7F7F7', tidy.opts=list(blank = FALSE, width.cutoff = 60)}
...
```{r, m7_4, echo=TRUE, eval=TRUE, collapse = TRUE, comment="", highlight=TRUE, background='#F7F7F7', tidy.opts=list(blank = FALSE, width.cutoff = 60)}
train_set %>%
  dplyr::count(movieId) %>% 
...
```{r, m7_5, echo=TRUE, eval=TRUE, collapse = TRUE, comment="", highlight=TRUE, background='#F7F7F7', tidy.opts=list(blank = FALSE, width.cutoff = 60)}
train_set %>%
  dplyr::count(movieId) %>% 
  left_join(movie_reg_avgs) %>%
  left_join(movie_titles, by="movieId") %>%
...
```{r, m7_6, echo=1:2, eval=TRUE, collapse = TRUE, comment="", highlight=TRUE, background='#F7F7F7', tidy.opts=list(blank = FALSE, width.cutoff = 60)}
...
```{r, m8_1, echo=TRUE, eval=TRUE, comment=""}
lambdas <- seq(0, 10, 0.25)
rmses <- sapply(lambdas, function(l){
  mu <- mean(train_set$rating)
  b_i <- train_set %>%
...
```{r, m8_2, echo=TRUE, eval=TRUE}
(lambda <- lambdas[which.min(rmses)])
```
```{r, m8_3, echo=FALSE, eval=TRUE}
...
```{r, m9_1, echo=TRUE, eval=TRUE, comment=""}
lambdas <- seq(0, 10, 0.25)
rmses <- sapply(lambdas, function(l){
  mu <- mean(train_set$rating)
  b_i <- train_set %>%
...
```{r, m9_2, echo=FALSE, eval=TRUE, comment=""}
mu <- mean(train_set$rating)
movie_reg_avgs <- train_set %>%
    group_by(movieId) %>%
    summarize(b_i = sum(rating - mu)/(n()+lambda), n_i = n())
...
```{r, mf_cv_all_models, echo=TRUE, eval=TRUE, warning=FALSE, comment=""}
Sys.time()
...
```{r, m10_1, echo=TRUE, eval=TRUE, comment=""}
# load("rdas/train_set.rda")
...
```{r, m10_2, echo=FALSE, eval=TRUE, comment=""}
mu <- mean(train_set$rating)
lambda <- lambda_opt
movie_reg_avgs <- train_set %>%
    group_by(movieId) %>%
...
```{r, m10_3, echo=FALSE, eval=TRUE, comment=""}
...
```{r, final_val_1, echo=TRUE, eval=TRUE, comment=""}
...
```{r, final_val_2, echo=FALSE, eval=TRUE, comment=""}
...
```{r, final_val_3, echo=FALSE, eval=TRUE}
