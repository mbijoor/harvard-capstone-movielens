...
```{r, nm_1, eval=TRUE, collapse = TRUE, comment="", highlight=TRUE, background='#F7F7F7', tidy=TRUE}
(mu_hat <- mean(train_set$rating))
```

If we predict all unknown ratings with $\hat\mu$ we obtain the following RMSE:

```{r, nm_2, eval=TRUE, collapse = TRUE, comment="", highlight=TRUE, background='#F7F7F7', tidy=TRUE}
(naive_rmse <- RMSE(test_set$rating, mu_hat))
```

Keep in mind that if we plug in any other number, we get a higher RMSE. For example:

```{r, nm_3, echo=1:8,  eval=TRUE, collapse = TRUE, comment="", highlight=TRUE, background='#F7F7F7', tidy=TRUE}
...
```{r, nm_4, echo=FALSE, eval=TRUE}
rmse_results <- tibble(Index = "1", Method = "Just the average", RMSE = naive_rmse)

save(rmse_results, file = "rdas/rmse_results.rda")

rm(mu_hat,naive_rmse)
```
```{r, echo=FALSE, eval=TRUE}
# rmse_results %>% knitr::kable()
  kable(rmse_results, "latex", escape=FALSE, booktabs=TRUE, linesep="", caption="RMSE Results\\label{tbl:rmse_results}") %>%
    kable_styling(latex_options=c("HOLD_position"), font_size=7)
```
